...
func xx512(res *[64]byte, b [64]byte) {
...
func Add(accum, operand *[64]byte) {
	// TODO: carry no more than one byte?
	var t uint16 = 0

	for i := 63; i >= 0; i-- {
		t = uint16(accum[i]) + uint16(operand[i]) + t
		accum[i] = byte(t & 0xff)
		t >>= 8
	}
}

func gn(N, h, m *[64]byte) {
	var tmp, K [64]byte
...
func hash(hash [64]byte, msg []byte, ) [64]byte {
	var v512, v0, Eps, N, m [64]byte
	v512[62] = 2
	for len(msg) > 64 {
		submsg := msg[len(msg)-64:]
		for i := 0; i < 64; i++ {
			m[i] = submsg[i]
		}
		msg = msg[:len(msg)-64]
...
func Hash64(msg []byte) [64]byte {
	var IV [64]byte
	return hash(IV, msg)
}

func Hash32(msg []byte) [32]byte {
	var IV [64]byte
	for i := 0; i < 64; i++ {
		IV[i] = 1
	}
